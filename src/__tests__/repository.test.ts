...
import { LocalAppRepository, localStorageKey } from '../data/LocalAppRepository';
...
describe('LocalAppRepository', () => {
  ...
  it('persists attendance and tomorrow flags with counts', () => {
    ...
    const reloaded = new LocalAppRepository(defaultAppConfig, window.localStorage);
  ...
  it('creates immutable operator logs on status update', () => {
    ...
    expect(snapshot.venues.find((venue) => venue.id === 'dream')?.state).toBe('busy');
  });
  ...
  it('persists personal notes and practice selection/completion', () => {
    ...
    expect(snapshot.practice.completedDayIndexes).toContain(defaultAppConfig.todayIndex);
