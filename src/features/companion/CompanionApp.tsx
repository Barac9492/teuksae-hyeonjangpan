...
import { LiveParkingPanel, LiveWorshipPanel, useLiveOperations } from './LiveOperations';
...
export function CompanionApp() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setLiveDay(liveEventDay()), 60_000);
  ...
  return <div className="tc-companion" lang="ko"><aside className="tc-desktop-note" aria-label="시안 설명"><span>WOORI CHURCH · AUTUMN 2026</span><h1>새벽의 마음을,<br />그대로.</h1><p>예배에 오기 전부터<br />함께 아침을 먹는 시간까지.</p><img src={crownImage} alt="공식 포스터에서 가져온 왕관과 두 손" /><small>{isPreview ? '디자인 검토용 미리보기입니다.' : '현장 현황 시범 운영 · 기도·사진·나눔은 서버에 접수되지 않습니다.'}</small></aside><div className="tc-app"><header className="tc-app-header"><button className="tc-brand" type="button" onClick={() => selectTab('worship')} aria-label="예배 첫 화면"><strong>우리</strong><span>분당우리교회</span></button><div className="tc-header-actions"><button className="tc-text-button" type="button" onClick={() => setModal({ type: 'poster' })}>행사 포스터 <span aria-hidden="true">↗</span></button><a className="tc-admin-link" href="/admin">관리자 로그인</a></div></header>{isPreview ? <div className="tc-demo-banner"><span>디자인 미리보기 · 실제 현황 아님</span><button type="button" onClick={() => setModal({ type: 'settings' })}>상황 바꿔보기</button></div> : <div className="tc-live-banner"><span>현장 현황 시범 운영</span><small>교회 공식 앱 승인 전 공개 안내</small></div>}<main id={mainId} ref={mainRef} tabIndex={-1}>
    <div hidden={tab !== 'worship'}>{isPreview ? <WorshipPanel venue={venue} setVenue={setVenue} stage={stage} stale={stale} eventDay={eventDay} goToParking={() => selectTab('parking')} goToSnack={() => { setSharingView('snacks'); selectTab('sharing'); }} showRoute={() => setModal({ type: 'route', venue })} /> : <LiveWorshipPanel venue={venue} setVenue={setVenue} operations={operations} crownImage={crownImage} eventDay={liveDay} goToParking={() => selectTab('parking')} goToSnack={() => { setSharingView('snacks'); selectTab('sharing'); }} />}</div>
    <div hidden={tab !== 'parking'}>{isPreview ? <ParkingPanel venue={venue} setVenue={setVenue} stage={stage} stale={stale} allFull={parkingFull[venue]} goToWorship={() => selectTab('worship')} /> : <LiveParkingPanel venue={venue} setVenue={setVenue} operations={operations} />}</div>
    <div hidden={tab !== 'prayer'}><PrayerPanel onPreview={(text, sharing) => setModal({ type: 'prayer', text, sharing })} /></div>
    <div hidden={tab !== 'sharing'}><SharingPanel eventDay={isPreview ? eventDay : liveDay} venue={venue} setVenue={setVenue} view={sharingView} setView={(next) => { setSharingView(next); if (mainRef.current) mainRef.current.scrollTop = 0; }} stories={visibleStories} onAddStory={addStory} onDeleteStory={(id) => setStories((current) => current.filter((story) => story.id !== id))} onHideStory={(id) => setHiddenStories((current) => new Set(current).add(id))} onMoreStories={() => setModal({ type: 'stories' })} /></div>
