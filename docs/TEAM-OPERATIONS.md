...
4. Sign in as the bootstrap superadmin and create `WOORIPARK` with role `parking` and `WOORISEAT` with role `space`. Accounts may be created inactive without passwords. Activating requires a stored password of at least 8 characters.
